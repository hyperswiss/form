...
<html lang="de-DE">
	<head>
		...
		<meta name="description" content="kontaktformular.com hidedefo"/>
   ...
   document.getElementsByTagName("BODY")[0].className += " safari";
		...
		// -------------------- DATEIUPLOAD START ----------------------
			if(0<$cfg['NUM_ATTACHMENT_FIELDS']){
				echo '<div class="row upload-row" style="background-image: url(img/border-right.png);background-position: 2.90rem center;-webkit-text-size-adjust:none;background-repeat: no-repeat;">
						<div class="col-sm-8">
		...
		if($cfg['Security_code']) { ?>
			<div class="row captcha-row <?php if ($fehler["captcha"] != "") { echo 'error_container'; } ?>" style="background-image: url('img/border-right.png');background-position: 2.90rem center;-webkit-text-size-adjust:none;background-repeat: no-repeat;">
				<div class="col-sm-8 <?php if ($fehler["captcha"] != "") { echo 'error'; } ?>">
					...
					<div>
						<img aria-label="Captcha" src="captcha/captcha.php" alt="Security code" title="kontaktformular.com-sicherheitscode" id="captcha" />
		...
		if($cfg['Security_question']) { ?>
			...
			<div class="row question-row <?php if ($fehler["q_id12"] != "") { echo 'error_container'; } ?>" style="background-image: url('img/border-right.png');background-position: 2.90rem center;-webkit-text-size-adjust:none;background-repeat: no-repeat;">
				<div class="col-sm-8 <?php if ($fehler["q_id12"] != "") { echo 'error'; } ?>">
					...
					<div aria-label="Security_question">
						<?php echo $q[1]; ?>
						<input type="hidden" name="q_id" value="<?php echo $q[0]; ?>"/>
					</div>					
					<input <?php if($cfg['HTML5_error_messages']) { ?> required <?php }else{ ?> onchange="checkField(this)" <?php } ?> aria-label="Antwort" id="answer" placeholder="Please answer the question. *" type="text" class="field<?php if ($fehler["q_id12"] != "") { echo ' errordesignfields'; } ?>" name="q" onclick="setActive(this);" onfocus="setActive(this);"/>
		...
		if(1==$cfg['Send_copy']) { ?>
			<div class="row checkbox-row" style="background-image: url('img/border-right.png');background-position: 2.90rem center;-webkit-text-size-adjust:none;background-repeat: no-repeat;">
		...
		if($cfg['DATA_PRIVACY_POLICY']) { ?>
			<div class="row checkbox-row <?php if ($fehler["data_protection"] != "") { echo 'error_container'; } ?>" style="background-image: url('img/border-right.png');background-position: 2.90rem center;-webkit-text-size-adjust:none;background-repeat: no-repeat;">
...
<hr style="height:0.10rem; border:none; color:#DADADA; background-color:#DADADA; margin-top:40px; margin-bottom:35px;" />
			...
			<div class="row" id="send">
				<div class="col-sm-8">
					...
					<br />
					<br />
					<input type="submit" class="senden" name="kf-km-en" value="Send" />
		...
		<?php if($cfg['Click_check']){ ?>
			<script type="text/javascript">
				...
				document.getElementsByName('kf-km-en')[0].addEventListener('mouseenter', chkspmkcfnk);
				document.getElementsByName('kf-km-en')[0].addEventListener('touchstart', chkspmkcfnk);
			</script>
		<?php } ?>
			<script type="text/javascript">
				...
				document.getElementsByName('kf-km-en')[0].addEventListener('click', setValidationStyles);
